...
function dePalabrasAFrase(palabras) {
  ...
  // Tu código:
  var palabraC = palabras[0]
  for(var i = 1; i < palabras.length;i++)
  {
    var palabraC = palabraC + " " + palabras[i]
  }
  return palabraC
}
...
function promedioResultadosTest(resultadosTest) {
  ...
  {
    resultadosTest[i] = Math.round(resultadosTest[i])
...
function empiezaConNueve(n) {
  ...
  //Escribe tu código aquí
  function empiezaCon(num,emp)
  {
    const numS = num.toString()
    const empS = emp.toString()
    if(numS.charAt(0) === empS)
    {
      return true
    }
    return false
  }
  return empiezaCon(n,9)
}
...
function mesesDelAño(array) {
  ...
  for (let index = 0; index < array.length; index++) 
  {
    ...
    if (index === array.length - 1 && mesesPedidos.length < 3)
    {
